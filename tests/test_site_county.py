...
class TestTheCountyCsv(CountyPageCase):
    ...
    def test_a_location_that_reads_as_a_formula_is_published_as_text(self):
        self.observe(detail("1", location='=HYPERLINK("http://x","Ballina")',
                            outageType="Planned", plannedOutageReason=" @SUM(1)"),
        ...
        # an unmapped reason, behind a space a spreadsheet would trim
        self.assertTrue(row["planned_reason"].startswith("'"), row["planned_reason"])

    def test_a_formula_behind_whitespace_is_defused_too(self):
        for text in (" =1+1", "\n=1+1", "\t@x"):
            self.assertTrue(render._cell(text).startswith("'"), repr(text))
        self.assertEqual(render._cell("Ballina"), "Ballina")
