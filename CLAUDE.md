...
| A storm can list more than a run can fetch: `ids_needing_detail` ranks never-fetched outages before re-checks, every detail is committed as it lands, SIGTERM from the service unit's 25-minute stop ends the loop and records the run as `cut_short` (exit 0, heartbeat sent, no webhook), and the pause is 500 ms. In list order a cut-short run re-fetched the same head every run and never reached the tail | `notes/storms.md` (2026-09-06) |
