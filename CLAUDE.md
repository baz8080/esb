...
| "Restored by ESB's estimate" is the share of faults, per outage not per customer, back within five minutes (`ESTIMATE_GRACE`, the row's own line) of the time ESB named, blank under five estimates (`MIN_ESTIMATES`). A tile beside "restored within 4 hours" on both app views and a column in the county month table; no all-time figure in the footer | `notes/design-alignment.md` § The site says how good ESB's estimates are (2026-09-05) |
