...
|---|---|---|---|---|
| 00 | Ask the grid the same question (intro) | - | drafted | 1,544 |
...
| 05 | What the page may claim | #1 | drafted | 2,024 |
...
| 08 | Closing: two feeds, two sites, one discipline | - | drafted | 2,733 |

Total ~24,500 words, two SVGs (`envelope-not-sum.svg`, `horizon.svg`), one mermaid flow
...
- **00** The water site's question grows a power sibling; the purging feed (112-min
  ...
  9 A counties, 1 outage past 24 h. AI process named once (93 of 138 commits co-authored).
...
- **08** Question answered with dates (both scales); can-say / cannot-say lists incl.
  attribution; the 15-row side-by-side table + the identical column; settled-decisions table
  (21 rows) in plain language; "collect first, interpret later, keep the bytes"; glossary of
  15 own + 3 borrowed boxes.
...
- A root README pointer to `writing/` is deliberately left for the publish decision, as the
  water series did.
- Anything landing after PR #29 needs a new chapter or an extension to 7b; the repo is moving
  roughly a pull request a day, so check `git log origin/main` before assuming the account is
  current.
