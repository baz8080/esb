...
| Faults past the charter's 24-hour compensation mark are the `Over 24 h` column of the county page's month table, read off the same payload row as the rest of it. Not a tile: the national tile was declined (2026-08-28) and a county tile would read 0 nearly everywhere | `notes/design-alignment.md` § The county table carries the 24-hour count the footer promised (2026-09-05) |
| An outage still listed at the last poll carries `ongoing` as the case record's twelfth field, ships its estimate, and reads "still out when last checked" plus what ESB has said (expected back by, past ESB's estimate of, no estimate published, judged against the horizon rather than the last sighting); no span, because its end is the horizon. Planned works keep their schedule wording. On a merged event `ongoing` and the estimate follow the record that ended it | `notes/design-alignment.md` § An outage still out says so (2026-09-05) |
| "Restored by ESB's first estimate" is the share of faults, per outage not per customer, back no later than five minutes (`ESTIMATE_GRACE`, one-sided, the row's own line) after the **first** time ESB named (`Outage.first_est`; four in five revisions come after a miss), blank under five estimates (`MIN_ESTIMATES`). The row keeps comparing against ESB's last word. A tile beside "restored within 4 hours" on both app views and a column in the county month table; no all-time figure in the footer | `notes/design-alignment.md` § The site says how good ESB's estimates are (2026-09-05) |
| The county page's "Where faults keep happening" card ranks ESB's own location names by faults over every month: ten rows, two faults or more, no links, because 222 of 422 names straddle more than one Census area | `notes/design-alignment.md` § The county page ranks its fault spots (2026-09-06) |
| Each county page links `c/<slug>.csv`: one row per merged event, oldest first, columns in `render.CSV_COLUMNS`, written for every county and kept out of the sitemap and the budget | `notes/design-alignment.md` § The CSV (2026-09-06) |
...
| The grade has three gates and they are different facts: **five observed days** is national (`observed_window` takes no county) and about the calendar, **five faults** is the county's own, and **`within is None`** is a county that cleared both and still has no fault that started in the month and has been restored. A fault is judged in the month it started, however long it ran. None counts "days with faults". It stays at five days: measured, that is a floor against a day-1 F, not a settling point | `notes/grading.md` § The five-day gate had nobody to explain it (2026-09-03); § A fault is judged in the month it started (2026-09-24) |
| Whichever gate withheld the letter, the sentence is printed **in the open**, not only in a chip's `title` a touch screen cannot open: under the county's name on `c/<slug>.html` and in the app's county view, under the month table for the older months, and above the county list as a count ("8 counties are not graded in September 2026: too few faults to grade fairly") because only the app sees all 26 at once. Every wording comes from `render.ungraded_reason`, mirrored once in `site.html` | `notes/grading.md` § The other two gates were hover-only (2026-09-07) |
| uisce and lifts need no such gate because they divide by time, which accrues whether or not anything happens; this site divides by the customers a fault reached, which is a sample and is `None` at zero faults | `notes/grading.md` § Why this site needs the gate |
...
| A month listed before any of its data exists says **no data yet**, never zeros: decided once by `model.month_watched` and shipped as `D.nodata`, so the county page and the app cannot disagree; nothing is counted or filed in it. It stays listed, because the month list follows the calendar | `notes/design-alignment.md` § A month the data has not reached says "no data yet" (2026-09-24) |
| Stored and computed in UTC, shown in Dublin time: months, day cells and gate dates are cut at Dublin midnight (as UTC instants, never Dublin-zoned datetimes), and case records stay UTC with only what is printed converted (`render._local`, `local()` in site.html) | `notes/grading.md` § Months and days are Dublin's (2026-09-24) |
| An outage still listed at the last poll is not judged on the charter, and a live fault runs to its last sighting, not to a passed estimate (planned works keep their schedule, issue #49) | `notes/grading.md` § An outage still listed; § A live fault runs to its last sighting (2026-09-24) |
...
| The fourteen towns named for their county are in `search.js` as `[name, slug]` when paged and render one row under the county's as `Sligo` + `town`; a bare county name (ESB's own string for a fault out in an ED) still stays out. The dedup that hid them was fixed upstream in statusui and the pin moved in the same PR | `notes/area-pages.md` § The county-named towns reach the box (2026-09-03) |
